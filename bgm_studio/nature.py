...
ループ結合しても継ぎ目が出てはいけない。そこで:
  ...
  1. ノイズ床は短い「巡回タイル」を作って敷き詰める。
     フィルタも巡回版を使うのでタイル境界は数学的に連続。
     フィルタ済みノイズには際立った過渡音が無いため、
     タイル周期が耳で認識されることはない。
...
def _bed(n: int, seed: int, kind: str, band: tuple[float, float] | None = None,
         ...
         tile_sec: float = 12.0, order: float = 2.0, sr: int = SR) -> np.ndarray:
    """巡回ノイズタイルを作り、巡回フィルタをかけて n まで敷き詰める"""
    L = _tile_len(n, tile_sec, sr)
    tile = dsp.looped_noise(L, np.random.default_rng(seed), kind)
    if band is not None:
        tile = dsp.circular_filter(tile, "bandpass", band[0], band[1], order=order)
    if lp is not None:
        tile = dsp.circular_filter(tile, "lowpass", lp, order=order)
    if hp is not None:
        tile = dsp.circular_filter(tile, "highpass", hp, order=order)
    ...
    for c in range(2):
        if window:
            base = _bed(n, seed * 7 + c + 1, "pink",
                        lp=1600.0 + 900.0 * intensity, hp=80.0, tile_sec=14.0)
        else:
            base = _bed(n, seed * 7 + c + 1, "pink", band=(200.0, 9000.0), tile_sec=14.0)
...
def fireplace(n: int, seed: int = 0, sr: int = SR) -> np.ndarray:
    ...
    for c in range(2):
        base = _bed(n, seed * 13 + c + 1, "brown", band=(45.0, 900.0), tile_sec=11.0)
...
def ocean(n: int, seed: int = 0, period: float = 9.0, sr: int = SR) -> np.ndarray:
    ...
    for c in range(2):
        ...
        low = _bed(n, seed * 17 + c + 1, "pink", lp=700.0, tile_sec=13.0)
        high = _bed(n, seed * 17 + c + 41, "pink", band=(900.0, 7000.0), tile_sec=13.0)
...
def wind(n: int, seed: int = 0, sr: int = SR) -> np.ndarray:
    ...
    out = np.zeros((n, 2), dtype=np.float32)
    for c in range(2):
        b1 = _bed(n, seed * 19 + c + 1, "pink", lp=350.0, tile_sec=12.0)
        b2 = _bed(n, seed * 19 + c + 1, "pink", lp=900.0, tile_sec=12.0)
        b3 = _bed(n, seed * 19 + c + 1, "pink", lp=2200.0, tile_sec=12.0)
...
def cafe_ambience(n: int, seed: int = 0, sr: int = SR) -> np.ndarray:
    ...
    L = _tile_len(n, 15.0, sr)
    for c in range(2):
        tile = dsp.looped_noise(L, np.random.default_rng(seed * 23 + c + 1), "pink")
