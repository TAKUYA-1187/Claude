...
PROGRESSIONS: dict[str, list[list[Chord]]] = {
    ...
    # ── 和風ローファイ: 下降ベースのモーダルな進行。
    #    和音を厚くしすぎるとペンタトニックの旋律とぶつかるので、
    #    sus2 / madd9 中心の開いた響きにする。
    "wafu": [
        _prog(("A", "madd9", 1), ("G", "add9", 1),
              ("F", "maj9", 1), ("E", "min7", 1)),
        _prog(("A", "min7", 1), ("D", "sus2", 1),
              ("F", "maj9", 1), ("G", "sus4", 1)),
        _prog(("A", "madd9", 2), ("F", "maj9", 2)),
        _prog(("D", "min9", 1), ("F", "maj7", 1),
              ("G", "add9", 1), ("A", "madd9", 1)),
    ],
    # ── クリスマス: 1950年代のジャズ・スタンダードの語彙。
    #    maj6 と dom7b9 が「あの頃のクリスマス」の匂いを出す。
    #    有名曲の旋律は借りない (Content ID で収益化できなくなる)。
    "christmas": [
        _prog(("C", "maj6", 1), ("A", "min7", 1),
              ("D", "min7", 1), ("G", "dom7", 1)),
        _prog(("C", "maj6", 1), ("E", "min7", 1),
              ("F", "maj6", 1), ("G", "dom7b9", 1)),
        _prog(("F", "maj6", 1), ("E", "min7", 1),
              ("D", "min7", 1), ("G", "dom9", 1)),
        _prog(("C", "maj9", 2), ("D", "min7", 1), ("G", "dom7", 1)),
    ],
    # ── ハロウィン: 短調 + 半音の軋み。怖がらせすぎない範囲で不穏に
    "halloween": [
        _prog(("A", "min", 2), ("F", "maj7", 1), ("E", "dom7b9", 1)),
        _prog(("A", "min", 1), ("D", "min", 1), ("E", "dom7b9", 1), ("A", "min", 1)),
        _prog(("A", "min", 2), ("G#", "dim7", 2)),
        _prog(("F", "maj7", 2), ("E", "dom7b9", 2)),
    ],
    # ── サイバーパンク: 短調のヴァンプ。動かないことが未来的
    "cyber": [
        _prog(("A", "min9", 4), ("F", "maj9", 4)),
        _prog(("A", "min9", 4), ("C", "maj9", 2), ("G", "add9", 2)),
        _prog(("D", "min9", 4), ("A", "min9", 4)),
    ],
    # ── ダークアカデミア: 古い図書室の物憂げな短調
    "academia": [
        _prog(("A", "min9", 1), ("D", "min9", 1), ("G", "dom7", 1), ("C", "maj7", 1)),
        _prog(("A", "min9", 1), ("F", "maj7", 1), ("B", "m7b5", 1), ("E", "dom7b9", 1)),
        _prog(("D", "min9", 2), ("E", "dom7b9", 2)),
        _prog(("C", "maj7", 1), ("B", "m7b5", 1), ("A", "min9", 2)),
    ],
    # ── 海/夜: 広がりのある add9 系
    "ocean": [
