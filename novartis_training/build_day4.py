...
■ 構成（計60分）
   オープニング 5分 ／ テーマ①「自分の範囲を考える」25分 ／
   テーマ②「動く理由を考える」25分 ／ エリア戦略・まとめ 5分
...
blocks = [("■ オープニング", "5分", "3回で何が変わったか／戦略的思考とは何か", GREEN),
          ("■ テーマ①「自分の範囲を考える」", "25分", "どこを見るか。大学の中からではなく、任された範囲から見てみる", NAVY),
          ("■ テーマ②「動く理由を考える」", "25分", "何のために動くか。成功像から逆算し、相手のWINも一緒に考える", NAVY),
...
txt(s, 0.6, 3.85, 12.15, 0.4,
    [P([R("戦略的思考といっても、難しい理論は使いません。今日考えるのは、この2つだけです。", 14, True, INK)],
       align=PP_ALIGN.CENTER)])
two = [("① どこを見るか", "視座", "大学の中から見る　→　自分の任された範囲から見る",
        "テーマ①「自分の範囲を考える」", NAVY, PALEB),
       ("② 何のために動くか", "順序", "情報から考える　→　成功像から逆算して考える",
        "テーマ②「動く理由を考える」", GOLD, YPALE)]
for i, (t, sub, d, tag, col, fill) in enumerate(two):
    ...
    card(s, x + 0.6, 5.99, 4.8, 0.42, [P([R(tag, 11.5, True, col)], align=PP_ALIGN.CENTER)],
         fill=WHITE, line=col, radius=0.2)
card(s, 0.6, 6.5, 12.15, 0.42,
     [P([R("戦略的思考 ＝ 「どこを見るか」×「何のために動くか」。この2つを考えると、同じ活動でも意味が変わります。", 12, True, DEEP)],
       align=PP_ALIGN.CENTER)],
...
# ================================================================ 12 テーマ①扉
s = add_slide()
section(s, "①", "自分の範囲を考える", "テーマ①　｜　どこを見るか　｜　25分（レクチャー10分・ワーク10分・共有5分）",
...
# ================================================================ 17 ワーク①
s = add_slide()
header(s, "WORK ①", "担当施設を、丸の大きさで描いてみる", "10分", kcolor=GREEN)
...
steps = [("STEP 1", "5分", "担当する大学・基幹病院を、丸で描く",
          "丸の大きさ＝あなたが「大事だ」と思う順。大きさに理由はいりません、感覚でOK"),
         ("STEP 2", "5分", "気になる丸に、ひと言メモを書く",
          "「ここが動けば範囲が変わる」「ここは会えていない」など、思ったことを自由に")]
y = 2.68
...
card(s, 0.6, 4.9, 8.05, 1.9,
     [P([R("描けたら、自分に1つだけ問いかけてください。", 13, True, INK)], space_after=8),
      ...
      P([R("一番大きくなくても、まったく問題ありません。それがあなたの範囲の実態です。この後、その理由をみんなで話します。", 11.5, False, GRAY)],
        line=1.25)],
...
card(s, 8.9, 1.78, 3.85, 5.02,
     [P([R("迷ったら、これだけ", 14.5, True, WHITE)], align=PP_ALIGN.CENTER, space_after=14),
      ...
      P([R("・線や矢印は無理に引かなくていい　思いついたら引く程度で十分", 12, False, WHITE)], space_after=12, line=1.25),
      P([R("・大きさに迷ったら　「ここが変われば範囲全体が変わるか？」で決める", 12, False, WHITE)], space_after=12, line=1.25),
...
# ================================================================ 19 テーマ②扉
s = add_slide()
section(s, "②", "動く理由を考える", "テーマ②　｜　何のために動くか　｜　25分（レクチャー10分・ワーク10分・共有5分）",
        "その情報は、何のために取りましたか？",
...
# ================================================================ 22 言語化する4つの問い
s = add_slide()
header(s, "THEME ②", "考え方の全体像 — 4つの問い ＋ 3", "25分", kcolor=NAVY,
       lead="到達目標「成功像・現状・課題・解決法を言語化できる」を問いにしたものです。全部を今日やる必要はありません。")
...
card(s, 0.6, 6.05, 12.15, 0.85,
     [P([R("①〜④が「自分の範囲をどうしたいか」、＋3が「そのために誰の・どの影響力を、どう活かすか」。", 13, True, DEEP)],
       align=PP_ALIGN.CENTER, space_after=4),
      P([R("このあとのワークでは、この中から3つ", 12.5, True, RED),
         R("（①いま一番変えたいこと／＋大学・医局のWIN／④両方が嬉しくなる打ち手）だけを考えます。形式も自由です。", 12, False, INK)],
        align=PP_ALIGN.CENTER, line=1.2)],
...
# ================================================================ 24 ワーク②
s = add_slide()
header(s, "WORK ②", "3つの問いを、考えてみる", "10分", kcolor=GREEN)
card(s, 0.6, 1.75, 8.05, 0.68,
     [P([R("さきほどの絵を横に置いて、3つの問いにメモするだけです。", 14, True, INK)], line=1.2)],
     fill=PALE, radius=0.08, pad=0.16)
qs3 = [("Q1", "3分", "私の範囲で、いま一番変えたいことは？",
        "「紹介が遅い」「あの病院で使われていない」など、ひと言でOK", GREEN),
       ("Q2", "3分", "その大学・医局は、何をやりたいと思っている？",
        "研修医が欲しい／論文を出したい／連携を強めたい…　分からなければ「？」でOK", NAVY),
       ("Q3", "4分", "両方が嬉しくなる打ち手は、何かありそう？",
        "思いつかなければ「ここが分からない」と書くだけで十分です", RED)]
y = 2.6
for tag, tm, q, hint, col in qs3:
    circle(s, 0.6, y + 0.08, 0.62, tag, fill=col, size=14)
    card(s, 1.4, y, 1.0, 0.78, [P([R(tm, 12, True, col)], align=PP_ALIGN.CENTER)],
         fill=PALE2, line=col, radius=0.15)
    card(s, 2.55, y, 6.1, 0.78,
         [P([R(q, 13, True, DEEP)], space_after=3), P([R(hint, 10, False, GRAY)], line=1.15)],
         fill=WHITE, line=LGRAY, radius=0.1, pad=0.14)
    y += 0.9
card(s, 0.6, 5.35, 8.05, 1.45,
     [P([R("答えが出なくても、まったく問題ありません。", 16, True, RED)], align=PP_ALIGN.CENTER, space_after=7),
      P([R("「ここが分からない」と気づけたら、それが今日一番の収穫です。次の面会で聞くことが決まったということですから。", 12, False, INK)],
        line=1.25)],
     fill=YPALE, radius=0.08, pad=0.2)
card(s, 8.9, 1.75, 3.85, 5.05,
     [P([R("手が止まったら", 14.5, True, WHITE)], align=PP_ALIGN.CENTER, space_after=14),
      P([R("Q1 → 絵の中で「詰まっている」と感じたところを探す", 12, False, WHITE)], space_after=12, line=1.25),
      P([R("Q2 →「先生は今、何をやりたいですか？」と聞けば分かること。今は想像でOK", 12, False, WHITE)], space_after=12, line=1.25),
      P([R("Q3 → 大学の7つの機能（教育・研究・臨床・人事・派遣・発信・講演会）から1つ選んでみる", 12, False, WHITE)], space_after=12, line=1.25),
      P([R("この3つが、そのまま次の共有のネタになります", 12, True, MINT)], line=1.25)],
     fill=DEEP, anchor=MSO_ANCHOR.TOP, radius=0.06, pad=0.2)
...
card(s, 0.6, 1.82, 12.15, 0.68,
     [P([R("3〜4人1組。発表会ではありません。テーマは用意しましたが、話したいことから自由にどうぞ。", 15, True, DEEP)],
       align=PP_ALIGN.CENTER)],
     fill=PALE, radius=0.1)
themes = [("A", "Q1で書いた「いま一番変えたいこと」を、話してみる", GREEN),
          ("B", "Q2の「大学・医局がやりたいこと」— 分かったこと・分からなかったこと", NAVY),
          ("C", "Q3の打ち手のアイデア。「こんなの、どうだろう？」で構いません", GOLD),
...
msgs = [(("どこを", "見るか"), "大学に固執しない。大学は範囲の中の「1施設」",
         "「大学を軸としたエリア攻略」ではなく、「エリア攻略のために大学を活かす」。", NAVY),
        (("何のために", "動くか"), "情報からではなく、成功像から",
         "成功像 → 大学・医局に求める役割の仮説 → 必要な情報 → 取りに行く → 修正。", GOLD),
        (("どこに", "置くか"), "打ち手は、WIN-WINの重なりに置く",
