...
ex_row = ["「関連病院の先生にも、うちの診断の考え方を知ってもらいたいんだよね」",
          "関連病院ごとに紹介の時期がばらつき、進行してから来る患者が多い",
          "資料を渡すのをやめ、先生を演者にした症例検討会を病院と共同で企画"]
x = 0.6
...
hyps = [("仮説A　地域の入口を担いたい", NAVY,
         ...
         "「どの施設・どの先生から、もっとご紹介いただきたいですか？」",
         "〇〇先生を演者にした地域向けの会を、病院と共同で企画する"),
...
card(s, 6.75, 2.58, 6.0, 0.55, [P([R("解決策の例（仮説に対応）", 14, True, WHITE)], align=PP_ALIGN.CENTER)],
     fill=GREEN, radius=0.1)
sol = ["A：先生を演者にした地域向けの会を、病院と共同で企画する",
...
quad = [("【成功像】　← Needs", RED, 0.5,
         ["〇〇先生を軸に、エリアの腎疾患患者が適切な時期にA病院へ集まっている"]),
        ("【現状・課題】　← Fact", GRAY, 0.5,
         ["腎生検は着任年度に2.5倍。ただし直近2年は96→99件で横ばい"]),
        ("【原因】　← 仮説", NAVY, 0.62,
         ["院内の診断力は上がったが、〇〇先生が地域へ発信する接点が",
          "まだ限られている（＝仮説A）"]),
...
card(s, 6.9, 1.75, 5.85, 0.45,
     ...
     fill=NAVY, radius=0.1)
nvs = ["〇〇先生を演者にした地域向け・エリア講演会の企画と運営支援",
...
wins3 = [("患者 WIN", "適切な時期に専門医へたどり着き、透析に至らない選択肢が残る", GREEN),
         ("〇〇先生 WIN", "紹介が増え、地域での存在感と診療実績が積み上がる", NAVY2),
