...
txt(s, 0.6, 1.44, 12.2, 0.32,
    [P([R("ある基幹病院の腎臓内科について、集めた情報9件です。①〜⑨を、下の3つのどこに置きますか。", 13.5, False, INK)])])
items = ["① 腎生検数が、1年で36件から90件に増えた",
         "② その後も96件、99件で推移している",
         "③ 血液浄化センター・臨床研究・学会発表のページがある",
         "④ 部長は前任地で、腎臓内科の立ち上げを経験している",
         "⑤「腎臓に問題がある患者様がいたら、いつでもご紹介ください」",
         "⑥「まず透析の話をせず、透析にならない方法を一緒に考えたい」",
         "⑦「透析を導入する医療ではなく、透析患者さんを一人でも減らしたい」",
         "⑧「どんな腎疾患の患者様にも、ベストな医療を偏りなく行いたい」",
         "⑨「安心して任せられる医師・医療を目指したい」"]
for i, it in enumerate(items):
    x = 0.6 + (i % 3) * 4.09
    y = 1.82 + (i // 3) * 0.8
    card(s, x, y, 3.9, 0.72, [P([R(it, 11.5, True, INK)], line=1.2)],
         fill=WHITE, line=LGRAY, radius=0.1, pad=0.13)
srows = [("Fact（事実）", GRAY), ("Wants（手段）", GREEN), ("Needs（状態）", RED)]
y = 4.32
for t, col in srows:
    card(s, 0.6, y, 2.6, 0.72, [P([R(t, 13.5, True, WHITE)], align=PP_ALIGN.CENTER)], fill=col, radius=0.1)
    shape(s, 3.35, y, 9.4, 0.72, fill=WHITE, line=LGRAY, line_w=1.0, radius=0.02)
    y += 0.8
txt(s, 3.35, 6.74, 9.4, 0.28,
...
txt(s, 0.6, 1.56, 12.15, 0.3,
    [P([R("公開情報から取れるWantsは、わずかでした。Wantsは、面談でしか増えません。", 12.5, True, DEEP)],
       align=PP_ALIGN.CENTER)])
card(s, 0.6, 1.98, 12.15, 0.95,
...
txt(s, 0.6, 1.42, 12.2, 0.3,
    [P([R("A病院の〇〇先生との面談で、こう聞こえたら ─ 同じ先生でも、言葉ごとにNeedsも打ち手も変わる。", 12, False, GRAY)])])
cols = [("出てきた Wants", "先生が言った言葉のまま", 3.3, GREEN),
        ("汲み取った Needs", "なぜ、そう言ったのか（＝仮説）", 3.1, RED),
        ("次の面会でやったこと", "MRが動いたこと", 2.8, NAVY2),
        ("つながった打ち手", "その先に形になったこと", 2.8, NAVY)]
x = 0.6
for t, sub, w, col in cols:
    card(s, x, 1.75, w, 0.6,
         [P([R(t, 12, True, WHITE)], align=PP_ALIGN.CENTER, space_after=2),
          P([R(sub, 9, False, WHITE)], align=PP_ALIGN.CENTER)],
         fill=col, radius=0.08)
    x += w + 0.05
rows = [("「もっと早い段階で腎生検をして、診断をつけて治療に入りたいんだよね」",
         "早期に生検・診断まで持っていける患者が、限られている",
         "資料を渡すのをやめ、どの段階の患者さんが多いかを一緒に整理",
         "「腎生検のタイミング」を先生が話す症例検討会を企画"),
        ("「腎臓に問題がある患者さんがいたら、いつでも紹介してほしい」",
         "紹介の入口が限られ、エリアから患者が集まってこない",
         "紹介元になり得る施設を、先生と一緒に地図に落とした",
         "周辺施設の医師を招き、先生を演者にした会を設計"),
        ("「まず透析の話をせず、透析にならない方法を一緒に考えたい」",
         "保存期で介入した成果を、院内外に示す場がない",
         "保存期で経過が変わった症例を、先生から聞き出した",
         "その症例を軸に、保存期CKDの検討会を先生中心に設計"),
        ("「前任地でやってきたことを、この病院でも生かしたい」",
         "立ち上げた診療体制と実績が、この病院にはまだない",
         "前任地で何を立ち上げたか、順番まで詳しく聞いた",
         "近隣の基幹病院・大学の腎臓内科医を紹介し、体制づくりへ")]
for r, cells in enumerate(rows):
    y = 2.5 + r * 1.0
    x = 0.6
    for i2, (t, sub, w, col) in enumerate(cols):
        card(s, x, y, w, 0.95, [P([R(cells[i2], 10.5, i2 in (1, 2), INK)], line=1.25)],
             fill=PALE2 if r % 2 else WHITE, line=LGRAY,
             anchor=MSO_ANCHOR.MIDDLE, radius=0.02, pad=0.12)
        x += w + 0.05
card(s, 0.6, 6.55, 12.15, 0.35,
     [P([R("Needsは仮説です。同じ言葉でも、次の面会で何を確かめるかで、打ち手は変わります。", 12, True, DEEP)],
       align=PP_ALIGN.CENTER)],
...
# ================================================================ 17 ④ 事例：公開情報
s = add_slide()
header(s, "④ 事例", "先ほどの9件は、すべてこの2つから取れます", kcolor=GOLD)
# --- 左：病院ホームページの実績ページ
...
# ================================================================ 18 ④ 事例：FactとWants
s = add_slide()
header(s, "④ 事例", "A病院 — 仕分けたFactとWantsを、並べて見る", kcolor=GOLD)
card(s, 0.6, 1.72, 6.0, 0.42,
...
card(s, 0.6, 1.68, 12.15, 0.52,
     [P([R("Wants　", 12.5, True, MINT),
       ...
       align=PP_ALIGN.CENTER)],
     fill=GREEN, radius=0.08)
hyps = [("仮説A　早期に生検・診断まで持っていきたい", NAVY,
         "早期に生検・診断まで持っていける患者が、限られている",
         "「どの段階でご紹介いただけると、生検まで持っていけますか？」",
         "「腎生検のタイミング」をテーマに、先生を演者にした症例検討会"),
        ("仮説B　透析を減らして評価されたい", GOLD,
         "透析を回避した成果を、院内外に示す場がない",
         "「腎生検が増えて、次に変わってほしいことは何ですか？」",
         "保存期CKDの症例検討会を、〇〇先生を中心に設計する"),
        ("仮説C　前任地の体制を再現したい", RED,
         "立ち上げた診療体制と実績が、この病院にはまだない",
         "「前任地でうまくいった仕組みで、再現したいものは？」",
         "近隣の基幹病院・大学の腎臓内科医とつなぎ、再現の土台をつくる")]
...
card(s, 0.6, 6.28, 12.15, 0.55,
     [P([R("打ち手はすべて、〇〇先生に対して。開業医は、その先にいる。", 17, True, YELL)],
       align=PP_ALIGN.CENTER)],
...
card(s, 0.6, 2.58, 6.0, 0.55, [P([R("仮説の例（A病院の事例より）", 14, True, WHITE)], align=PP_ALIGN.CENTER)],
     fill=NAVY, radius=0.1)
hyp = ["A：もっと早期に生検・診断まで持っていきたいのではないか",
...
card(s, 6.75, 2.58, 6.0, 0.55, [P([R("解決策の例（仮説に対応）", 14, True, WHITE)], align=PP_ALIGN.CENTER)],
     fill=GREEN, radius=0.1)
sol = ["A：「腎生検のタイミング」をテーマにした会を、先生と企画する",
       "B：保存期CKDの症例検討会を、先生を中心に設計する",
       "C：近隣の基幹病院・大学の腎臓内科医とつなぐ",
       "先生の症例を、エリア・全国の講演会で発信する",
...
# ================================================================ 20 ⑥ 事例：A病院②
s = add_slide()
header(s, "⑥ 事例", "A病院の〇〇先生に対して、何ができるか（仮説Aで4Sに）", "5分", kcolor=GREEN)
quad = [("【成功像】　← Needs", RED, 0.5,
         ["早期に生検・診断がつき、治療介入で透析に至る患者さんが減っている"]),
        ("【現状・課題】　← Fact", GRAY, 0.5,
         ["腎生検は着任年度に2.5倍。ただし直近2年は96→99件で横ばい"]),
        ("【原因】　← 仮説", NAVY, 0.62,
         ["院内の生検体制は整ったが、早期に紹介される患者がまだ限られている",
          "（＝仮説A）"]),
        ("【解決策】　← 〇〇先生への打ち手", GREEN, 1.05,
         ["・「腎生検のタイミング」をテーマに、先生を演者にした症例検討会",
          "・近隣の基幹病院・大学の腎臓内科医とつなぎ、先生を軸の会に",
          "・紹介元になり得る施設のマッピングを、一緒に整理する"])]
y = 1.72
...
card(s, 6.9, 1.75, 5.85, 0.45,
     ...
     fill=NAVY, radius=0.1)
nvs = ["「早期診断・早期介入」をテーマにした会の企画と運営支援",
       "近隣の基幹病院・大学の腎臓内科医との接点づくり（エリアマッピング）",
       "生検で確定診断がついた症例を、共有する場をつくる"]
...
wins3 = [("患者 WIN", "早期に確定診断がつき、治療介入で透析に至らない選択肢が残る", GREEN),
         ("〇〇先生 WIN", "早期の紹介が増え、生検・診療の実績が積み上がる", NAVY2),
...
card(s, 0.6, 6.35, 12.15, 0.48,
     [P([R("担当は大学・基幹病院。その先生のやりたいことを、エリアで実現できる形にする。", 14, True, DEEP)],
       align=PP_ALIGN.CENTER)],
     fill=YPALE, line=YELL, radius=0.1)
...
txt(s, 0.6, 1.62, 12.15, 0.34,
    [P([R("唯一の正解ではありません。なぜそう置いたかを説明できることが目的です。⑥⑧は割れやすい項目です。", 12.5, False, GRAY)],
       align=PP_ALIGN.CENTER)])
ans = [("Fact（事実）　4件", GRAY,
        ["① 腎生検数が、1年で36→90件に増えた",
         "② その後も96件、99件で推移している",
         "③ 血液浄化センター・臨床研究・学会発表のページ",
         "④ 部長は前任地で、立ち上げを経験している"],
        "→ 仮説の材料。単独では打ち手にならない"),
       ("Wants（手段）　2件", GREEN,
        ["⑤ いつでもご紹介ください",
...
for i, (t, col, lines, note) in enumerate(ans):
    ...
    card(s, x, 2.68, 3.9, 2.35,
         [P([R(l, 11.5, False, INK)], line=1.25, space_after=9) for l in lines],
         fill=WHITE, line=LGRAY, anchor=MSO_ANCHOR.TOP, radius=0.1, pad=0.17)
    card(s, x, 5.13, 3.9, 0.62, [P([R(note, 12, True, DEEP)], line=1.25)],
         fill=PALE2, radius=0.1, pad=0.14)
card(s, 0.6, 5.9, 12.15, 0.9,
     [P([R("公開情報から取れたWantsは、わずか2件。Wantsは、面談でしか増えません。", 15, True, YELL)],
       align=PP_ALIGN.CENTER, line=1.25)],
     fill=DEEP, radius=0.1, pad=0.18)
