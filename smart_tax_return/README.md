...
| 知りたいこと | 見る場所 |
|---|---|
| **いま何をすればいいか** | [`records/2026-07-09.md`](records/2026-07-09.md) — 進行中。7・8月分の収集と、1-3月分Amazon PDFの追加提出 |
| 前四半期の状態 | [`records/2026-04-06.md`](records/2026-04-06.md) — 4〜6月分は提出済み・チェック結果待ち。未解決1件あり |
| 次の期限 | 2027年1月10日（7〜12月分・`.07-12月分(1月10日まで厳守)`） |
| **制度対応（期限あり）** | [`records/2027-aoiro-75man.md`](records/2027-aoiro-75man.md) — 青色申告特別控除が令和9年分から75万円へ。届出書の提出とMF側の準備を**2026年内**に |

> ⚠️ **実作業はブラウザとOneDriveのある環境で行う。** このリポジトリのセッション（Claude Code on the web）は
> ユーザーのMac・OneDrive・Chrome拡張・マネーフォワードのいずれにも到達できない。
> Drive操作や明細ダウンロードを伴う作業は、デスクトップ／チャット側のセッションで
> `dist/smart-tax-return.skill` をインストールして実行する。このリポジトリは
> **手順書と作業記録の置き場**として使う。
