...
class YahooParser(BaseParser):
    ...
    def parse(self, uid: int, subject: str, html_body: str) -> Receipt:
        """Yahoo ショッピングの注文確認メールを解析して Receipt を返す"""
    ...
    def _extract_order_id(self, text: str, soup: BeautifulSoup) -> str:
        # 注文番号パターン: "注文番号" や "order ID" の後に続く数字列
        patterns = [
            r"注文番号[：:\s]*([A-Za-z0-9\-]+)",
            r"オーダー番号[：:\s]*([A-Za-z0-9\-]+)",
            r"ご注文番号[：:\s]*([A-Za-z0-9\-]+)",
        ...
        for a in soup.find_all("a", href=True):
            m = re.search(r"orderId=([A-Za-z0-9\-]+)", a["href"])
            if m:
                return m.group(1)

        raise ParseError("Yahoo: 注文番号を抽出できませんでした")

    def _extract_order_date(self, text: str, soup: BeautifulSoup) -> date:
        patterns = [
            r"注文日[：:\s]*(\d{4})[年/\-](\d{1,2})[月/\-](\d{1,2})",
            r"ご注文日[：:\s]*(\d{4})[年/\-](\d{1,2})[月/\-](\d{1,2})",
            r"(\d{4})[年/\-](\d{1,2})[月/\-](\d{1,2})日?\s*(?:のご注文|に注文)",
        ]
        for pattern in patterns:
        ...
        raise ParseError("Yahoo: 注文日を抽出できませんでした")
    ...
    def _extract_items(self, soup: BeautifulSoup, text: str) -> List[ReceiptItem]:
        ...
        if not items:
            m = re.search(r"商品名[：:\s]*(.+?)(?:単価|数量|小計)", text, re.DOTALL)
            if m:
                items.append(ReceiptItem(
    ...
    def _extract_amounts(
        ...
        subtotal = self._find_amount(text, ["小計", "商品合計", "商品代金"])
        tax_amount = self._find_amount(text, ["消費税", "税額", "内税"])
        shipping_cost = self._find_amount(text, ["送料", "配送料", "送料・手数料"])
        total = self._find_amount(text, ["合計金額", "お支払い合計", "請求金額", "総合計"])
